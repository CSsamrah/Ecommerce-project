...
export const totalTransactions = asyncHandler(async (req, res) => {
    try {
        ...
        const secondhandCount = await pool.query("SELECT COUNT(*) AS count FROM order_item WHERE item_type = 'secondhand'");
        const totalProductSales = await pool.query("SELECT COUNT(*) AS count FROM order_item WHERE item_type = 'product'");
...
export const revenueBySeller = asyncHandler(async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT p.user_id AS seller_id, u.name AS seller_name, SUM(o.total_price) AS total_revenue
            FROM orders o
