...
const getAllCategoies=asyncHandler(async(req,res)=>{
    ...
    limit=parseInt(limit) || 16;  //default 16 categories per page
    let offset=(page-1)*limit;

    const allCategories=await pool.query("SELECT * FROM category LIMIT $1 OFFSET $2",[limit,offset]);
    const totalCategories=parseInt(allCategories.rows.length);
    if(!allCategories.rows.length){
        throw new ApiError(404,"No categories found");
    }
    return res.status(200).json(new ApiResponse(200,{
        categories:allCategories.rows,
        totalCategories,   //shows the total no of categories fetched
        currentPage:page,
        totalPages:Math.ceil(totalCategories/limit),  //shows the total no of pages
    },
...
export {createCategory,getSingleCategory,getAllCategoies,updateCategory,deleteCategory}